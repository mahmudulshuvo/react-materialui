...
import { citiesCount, regionsCount } from "../../utils/demo-data/chart-data";
...
const barSeriesForCity = regionCities =>
    Object.keys(regionCities[0]).reduce((acc, item, index) => {
        if (item !== "year") {
            acc.push(
                <BarSeries
                    key={index.toString()}
...
const gridDetailContainerBase = data => ({ row, classes }) => {
    ...
    return (
        <div className={classes.detailContainer}>
            <h5 className={classes.title}>{`Economics of ${row.region}`}</h5>
            <Paper className={classes.paper}>
                <Chart data={regionCities} height={300}>
                    <ArgumentAxis type="band" tickComponent={nullComponent} />
                    <ValueAxis
